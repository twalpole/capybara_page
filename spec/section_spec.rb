...
class Section < CapybaraPage::Section; end
...
describe CapybaraPage::Page do
  describe '.section' do
    ...
    describe '.section with class and block' do
      class SingleSection < CapybaraPage::Section
        element :single_section_element, '.foo'
      end

      class PageWithSections < CapybaraPage::Page
      ...
      before do
        allow(subject).to receive(:find).and_return(:element)
      end
    ...
    context 'second argument is not a Class and a block given' do
      class PageWithAnonymousSection < CapybaraPage::Page
    ...
    context 'second argument is not a Class and no block given' do
      ...
      let(:message) do
        'You should provide descendant of CapybaraPage::Section class or/and a block as the second argument.'
      end
    ...
    context 'default search arguments' do
      class PageWithSectionWithDefaultSearchArguments < CapybaraPage::Page
        class SectionWithDefaultArguments < CapybaraPage::Section
      ...
      context 'when search arguments provided during the section definition' do
        ...
        it 'returns the search arguments for a section' do
          expect(page).to receive(:find).with(*search_arguments).and_return(double(Capybara::Node::Element))
      ...
      context 'with niether default search arguments nor search arguments provided' do
        it 'should raise ArgumentError' do
          expect do
            class ErroredPage < CapybaraPage::Page
...
describe CapybaraPage::Section do
  let(:section_without_block) { CapybaraPage::Section.new(Page.new, locator) }
  let(:locator) { instance_double('Capybara::Node::Element') }
  let(:section_with_block) { CapybaraPage::Section.new(Page.new, locator) { 1 + 1 } }

  describe '#default_search_arguments' do
    class BaseSection < CapybaraPage::Section
  ...
  describe '#execute_script' do
    it 'delegates through root_element' do
      expect(locator).to receive(:execute_script).with('JUMP!')
  ...
  describe '#evaluate_script' do
    it 'delegates through root_element' do
      expect(locator).to receive(:evaluate_script).with('How High?')
      section_without_block.evaluate_script('How High?')
    end
  end

  describe '#parent_page' do
    let(:section) { CapybaraPage::Section.new(page, '.locator') }
    let(:deeply_nested_section) do
      CapybaraPage::Section.new(
        CapybaraPage::Section.new(
          CapybaraPage::Section.new(
    ...
    it 'returns the parent of a section' do
      ...
      expect(section.parent_page).to be_a CapybaraPage::Page
    end

    it 'returns the parent page of a deeply nested section' do
      expect(deeply_nested_section.parent_page.class).to eq(Page)
      expect(deeply_nested_section.parent_page).to be_a CapybaraPage::Page
    end
    ...
    it 'responds to Capybara methods' do
      pending "I'm not convinced this fully makes sense"
      expect(section).to respond_to(*Capybara::Session::DSL_METHODS)
    end
  end

  describe 'page' do
    subject { CapybaraPage::Section.new('parent', root_element).page }
