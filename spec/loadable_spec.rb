...
describe CapybaraPage::Loadable do
  let(:loadable) do
    Class.new do
      include CapybaraPage::Loadable
    end
  end
  ...
  describe '#when_loaded' do
    ...
    it 'raises an exception if any load validation fails' do
      ...
      end.to raise_error(CapybaraPage::NotLoadedError, /no reason given/)
    ...
    it 'raises an exception with a specific error message if available when a load validation fails' do
      ...
      end.to raise_error(CapybaraPage::NotLoadedError, /all your base are belong to us/)
    end
    ...
    it 'raises an error immediately on the first validation failure' do
      ...
      end.to raise_error(CapybaraPage::NotLoadedError)
